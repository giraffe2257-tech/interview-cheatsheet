    ...
    items: [
      {
        q: '可以用中文寫 prompt 嗎？',
        ja: `
実装に入る前に、一点だけ確認させてください。AIエージェントへのプロンプトは、普段は中国語で書いていて、そのほうが細かいニュアンスまで速く指示できます。今日もプロンプトだけ中国語、もしくは英語で書かせていただいてもよろしいでしょうか。

**必ず続けて言う（保護の一言）**

考えていることの説明は、すべて日本語でお話しします。書きながら、どういう指示を出しているかも日本語で声に出しますので、画面が中国語でも中身は追えるようにします。コード内のコメントや画面上の文言も日本語で書きます。

---

#### 「なぜ中国語なんですか」と聞かれたら

母語なので、指示のニュアンスを詰めるのが一番速いからです。30分という制限の中では、プロンプトを書く時間を短くして、動くものを出す時間に回したいと考えています。
...
`,
        zh: `
**問的時機：討論結束、要開始動手的交界（第 15 分附近）。不要等到動手之後才問。**
...
|---|---|
| 只講第一段 | 有機會被讀成「日文不行」 |
| 加上第二段 | 被讀成效率判斷，而且解掉面試官「看不懂你畫面」的疑慮 |

**「邊寫邊用日文講我下了什麼指令」這句一定要留。** 邊做邊講本來就是這一關的評分項，這句同時解掉兩件事。

被對方要求改用日文時，立刻照做，不要爭辯。這時候順從比效率重要。
...
最後一句是**宣告不是提問，一定要自己主動講**。它同時展現準備度，又劃清界線：我準備的是白紙，不是答案。
`
      },
      {
...
| 中文 | 日文 |
|---|---|
| ★ 主要用戶是誰？ | 主なユーザーは誰を想定していますか？ |
| ★ 讓他察覺之後，希望他做什麼？ | 気づかせた後、ユーザーに何をしてほしいですか？ |
...
| 中文 | 日文 |
|---|---|
| ★ 有沒有不能碰的部分？ | 触ってはいけない部分はありますか？ |
| ★ 今天優先會動還是好看？ | 動くものと見た目、どちらを優先しますか？ |
| ★ 假資料可以嗎？ | データはダミーで良いですか？ |
...
| ★ 沒有資料的人打開會怎樣？ | データがないユーザーの場合はどうなりますか？ |

#### ④ Data

| 中文 | 日文 |
|---|---|
| ★ 資料從哪來？Server 還是本地？ | データはサーバー経由ですか、ローカルですか？ |
| ★ **文章上有哪些標籤？** | 記事にはどんなラベルが付いていますか？ |
| ★ 閱讀紀錄拿得到嗎？拿得到什麼？ | 閲覧履歴は取れますか？何が取れますか？ |
...
| 中文 | 日文 |
|---|---|
| ★ 這個功能放在哪裡？ | この機能はどこに置く想定ですか？ |
| 主動彈出還是用戶自己去看？ | 能動的に出しますか、ユーザーが見に行きますか？ |
`
      },
      {
        q: '第 8 分鐘要宣告的那句話',
